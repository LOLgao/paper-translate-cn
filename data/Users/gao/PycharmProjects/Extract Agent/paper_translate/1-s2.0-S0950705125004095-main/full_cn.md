...
现有的异构图社区检测方法主要关注同质性异构图，通常使用GNN和HGNN。在具有异质性的异构图网络中，由元路径连接的两个节点往往具有不同的标签，现有方法会导致来自不同社区的节点表示坍缩。为了解决结构异质性带来的挑战并增强同质性强度，提出了一种以同质性为导向的多级结构融合无监督社区检测方法，称为SFCDH。该方法在各种同质性和异质性异构图网络中展现出强大的泛化能力。具体而言，SFCDH包含两个模块：基于同质性比率采样的层次结构融合模块和基于生成式与对比学习的无监督图表示学习模块。第一个模块基于网络同质性构建细粒度的高阶融合图，并通过层次抽象树将社区划分为多粒度结构，以提高模型对不同元路径子图的泛化能力。随后，第二个模块利用基于社区原型的对比损失来指导图生成动态掩码，并通过图自编码器增强SFCDH学习属性和复杂语义的能力。最终，将元路径级别和树级别的节点嵌入相结合，实现多级结构融合以进行无监督社区检测。在六个公开数据集上的大量实验表明，SFCDH优于最先进的基线方法。值得注意的是，在异质性异构图IMDB中，SFCDH在准确率上比第二名模型高出$1 1 \%$，进一步验证了其有效性。

# 1. 引言

现实世界中的复杂系统可以抽象并建模为复杂网络，例如社交网络[1]、生物信息网络[2]和交通网络[3]。复杂网络中的节点代表某一类实体，而连接边则表示两个实体之间的相互关系。社区检测旨在识别复杂网络中的不同社区，其中内部节点连接紧密，而外部节点之间的连接稀疏。

异构图由多种类型的节点和边组成，与同构图相比，具有更丰富的结构和语义信息。例如，在图1(a)中，作者节点A1和A2之间没有直接边，但通过元路径APA相连，表明他们共同撰写了一篇论文。使用传统的同构图社区检测方法很难将这两个节点划分到同一社区。因此，异构图对社区检测提出了重大挑战，突显了针对这些复杂结构定制专门方法的必要性。

现有的大多数异构图社区检测方法涉及提取各种元路径并将其视为同构图，然后使用标准图嵌入技术（如GNN）通过学习节点嵌入来识别图中的社区。据此，Luo等人[4]构建了一种基于上下文路径的GNN来检测异构图，并应用上下文路径捕获节点之间的高阶关系。Zhao等人[5]通过提出一种具有层次注意力机制的图自编码器，聚合异构图结构信息，应用非负矩阵分解学习图社区语义信息，并最终以统一方式优化两者。Tian
...
等人[6]通过掩码每个同构子图的属性、拓扑和位置信息，计算重构损失并学习节点嵌入。上述方法中使用的GNN、HGNN等基于同质性假设，即同一社区的节点更可能相互连接。通常，异构图并不总是满足同质性假设，因为从元路径导出的同构子图也可能包含大量异质边。

在图1(b)中，通过元路径获得的同构子图由作者节点组成，这些节点可以是同质或异质的。实线表示同质边，表明连接的两个节点属于同一社区，而虚线表示异质边，连接来自不同社区的节点。异质图包含许多异质边，而同质图则很少。异质图中过多的跨类边会导致不同社区节点的特征在聚合后混合，使其难以区分[7]。如图1(b)所示，对于难以区分的边界节点，同质结构为其聚合特征提供的信息比原始节点特征更有价值。相反，缺乏同质性（即异质性）是GNN和HGNN在异质图上表现不佳的重要原因。边界节点通常拥有更多异质邻居而非同质邻居。由于异质边连接不同类别的节点，它们可能导致节点嵌入混合且难以区分，模糊社区之间的边界。
...
M1：在存在缺失或噪声标签的情况下，处理异构图中的复杂结构和语义信息变得尤为困难。无监督方法对于管理这些图中多种类型的节点至关重要。通过利用可用的丰富语义信息，我们可以提升社区检测的有效性。

M2：在异质性异构图的不同社区节点之间避免表示坍塌，对于保持这些多样化节点的独特特征和关系至关重要。在处理异构图时，实施能够最小化结构异质性同时增强同质性的策略具有重要意义。

# 1.2. 贡献

本文提出了一种无监督社区检测框架，该框架能够破坏结构异质性并增强同质性强度。该框架通过生成元路径级节点嵌入和层次树级节点嵌入，实现多层级结构融合，并利用生成式学习与对比学习完成无监督表示学习。据我们所知，本文首次提出了针对同质性与异质性异构图的无监督社区检测方法。

本文的主要贡献如下：

（1）提出了一种统一框架，包含基于同质性比率采样的层次结构融合模块（SFR）和用于生成式与对比学习的无监督图表示学习模块（UGC）。该模型在多种同质性与异质性异构图上展现出强大的泛化能力；
（2）所提出的SFR模块通过不同元路径实现自适应高阶子图聚合，构建细粒度高阶图。此外，利用结构熵从该高阶图中构建抽象树，以学习稳定的节点嵌入。该模块在增强同质性边影响力的同时削弱异质性边的影响；
（3）所提出的UGC模块将对比学习与生成式学习相结合，利用基于社区原型的对比损失与动态图掩码自编码器，计算重构前后特征与结构的损失。在模型训练过程中，该模块动态增强节点特征与异构图语义的学习；
（4）在包含同质性与异质性异构图的6个公开异构图数据集上进行的大量实验表明，与11个最先进基线模型相比，SFCDH方法具有有效性和泛化性。值得注意的是，在异质性异构图数据集IMDB上，SFCDH在准确率上比第二名模型高出$1 1 \%$，进一步验证了其有效性。

本文其余部分组织如下：第2节回顾相关工作。第3节全面概述预备知识。第4节介绍所提出的模型。第5节报告实验结果与分析。最后，第6节总结本文结论与未来工作。

# 2. 相关工作

# 2.1. 同质图社区检测

社区检测作为网络研究的热点，近几十年来受到了广泛关注。传统的社区检测方法侧重于根据网络节点的局部拓扑结构将其划分为不同的社区。Shi等人[8]提出了一种改进的成对约束非负对称矩阵分解方法，该方法利用从真实社区信息中生成的成对约束来提高社区检测性能。He等人[9]首次将马尔可夫随机场应用于网络分析，将不规则网络的结构特性编码为能量函数，从而通过最小化能量函数得到最合适的社区结构。Gianluca Bonifazi等人[10]利用传播者中心性来检测社交网络中不同社区间的信息扩散。为了进一步解决动态性问题，他们研究了在非危险和危险挑战期间社区演化模式中的惊喜因素[11]。Kang等人[12]提出了一种基于多邻域的局部搜索策略，该策略具有动态分组机制，并重新定义了个体更新规则。该方法将社区检测技术应用于农业中的机器人任务分配。Kong等人[13]通过对称分解邻接矩阵和属性矩阵，利用丰富的多域信息优化社区隶属矩阵，以检测复杂属性网络中的社区。多数据的联合分解导致计算复杂度较高。Li等人[14]利用模体，通过提取每层的高阶交互，将多路网络聚合为单层多路网络进行社区检测。

随后，许多研究者利用深度学习来解决社区检测问题。Liu[15]等学者提出利用高阶邻近性和重叠社区模块度函数来学习网络的潜在社区结构。Hao等人[16]通过多个图自编码器将层次属性信息与拓扑关系有机结合，克服了特征不足的问题。然而，上述方法可能导致过度划分或欠划分问题。Sun等人[17]提出了一种图信息最大化社区模型，该模型利用网络内的社区信息来学习节点嵌入。Li等人[18]将图对比学习与图卷积网络相结合，该方法考虑了社区信息，并以端到端的方式联合分析社区检测和节点嵌入。

然而，上述方法侧重于同构图，无法处理具有多种节点类型和关系的异构图，也未能考虑异质性。

# 2.2. 异构图社区检测

目前，针对异质网络社区检测的研究较少，且研究内容零散、方法多样，难以形成统一的体系。现有的异质网络社区检测方法主要分为两类：传统异质网络社区检测方法和基于深度学习的异质图神经网络聚类方法。总体而言，传统异质网络社区检测方法的泛化能力不强，通常针对特定网络结构设计。Sun等人[19]提出了一种基于概率的二部图算法。Li等人[20]针对特定网络结构提出了基于矩阵分解的方法。基于深度学习的聚类方法根据其对网络中多样关系的处理方式，大致可分为两类：元路径方法和自适应方法。前者依赖预定义的元路径来捕获各种语义关系，然后利用神经网络表示节点信息以生成节点嵌入，再基于这些嵌入执行特定的聚类任务。Wang等人[21]提出了HAN模型，该模型结合注意力机制和元路径结构来聚合最终的节点嵌入。Zhang等人[22]提出了HetGNN模型，该模型使用LSTM作为聚合器生成节点级嵌入，但捕获长距离关系的能力较弱。Yun等人[23]提出了GTN模型，该模型通过多通道卷积学习不同类型的元路径，并将多个节点嵌入拼接作为最终的节点嵌入，但该方法消耗大量内存。自适应方法则挖掘潜在的高阶结构以捕获关系信息，并引导节点特征的聚合。He等人[24]将异质社区规模与SBM结合用于链接社区检测。Zhao等人[25]利用元路径构建多个同质子图，在每个同质子图上递归地使用社区检测方法构建层次图，并生成保留社区语义的节点嵌入。Wei等人[26]通过随机游走，将局部节点特征以及全局社区和组织信息嵌入到节点嵌入中，并联合考虑社区、组织和属性信息，从而同时检测社区和组织结构。

然而，上述方法可能需要高质量的输入数据，尤其是在节点属性缺失或数据存在噪声的情况下，这可能导致结果出现偏差或不准确，无法处理属性缺失或含有噪声的真实网络节点。因此，我们考虑将图掩码自编码器与对比学习相结合，利用动态掩码策略扰动原始图，以无监督方式提升模型对图的学习能力。此外，上述方法忽略了复杂异质图中相邻节点具有不完全相关的不同特征（即结构异质性）。上述所有方法均基于同质性假设，无法处理具有异质性的异质图。

# 2.3. 异质性图表示学习

异质性图在现实生活中也普遍存在，从人际关系到蛋白质基因的科学研究。在异质性图中，关注点在于节点与其邻居之间的对比，突显了不同类别节点之间建立连接的倾向。最近几项研究致力于扩展图神经网络（GNN）以有效处理异质性图。Yang等人[27]采用了一种多样化的消息传递框架，为每个属性分配传播权重，并针对异质性图调整了GNN。Li等人[28]通过聚合全局信息学习了节点的潜在同质性信息。Chien等人[29]自适应地学习了同质性和异质性节点权重，并联合学习了节点特征和拓扑结构。Wu等人[30]使用自适应深度图卷积技术实现了局部高阶邻域的自适应聚合。针对药物-疾病网络的异质性，Liu等人[31]提出了一种新颖的结构增强线图卷积网络，用于学习结合结构信息的药物-疾病对的集成表示。此外，该方法存在较大的内存和时间开销。

尽管上述方法缓解了异质性同构图的问题，但在一定程度上，它们依赖于先验知识，例如用于训练的标签。因此，它们适用于半监督社区检测，但不适用于无监督社区检测，也无法处理具有多种节点和边类型的异构图。

在解决过度平滑问题时，Zheng等人[32]从拓扑空间角度引入了一种图互补卷积方法。他们主张生成互补图以增强原始图数据中缺失的结构细节，无论图结构是同质性还是异质性。Wen等人[33]基于图同质性，通过考虑低频和高频信号来增强节点嵌入。他们设计了一种图联合聚合矩阵，利用节点特征和邻接关系使这些信号更易区分。

上述大多数方法首先从拓扑空间入手，并处理邻域数据的处理方式。它们忽略了嵌入在高阶连接中的额外详细信息。然而，在模型训练过程中，没有一种方法充分评估相互连接的节点是否属于同一类别，从而限制了它们动态调节传播数据重要性的能力。这一限制可能阻碍模型的适应性，并损害其整体泛化能力。
...
本节定义了与本文相关的一些重要概念，并指出了本文试图解决的问题。表1给出了一些符号和说明。
...
定义2（元路径）。元路径 $\phi ~ \in ~ \phi$ 是HIN中连接不同类型节点到不同类型边的一种关系，定义为 $V _ { 0 } \xrightarrow { R _ { 0 } } V _ { 1 } \xrightarrow { R _ { 1 } } \cdots \xrightarrow { R _ { n } } V _ { n + 1 }$ ??1←←←←←←←←←→ . ????←←←←←←←←←←←→ ????+1。?? 表示节点之间的关系类型，而 $R _ { i }$ 表示节点 $V _ { i }$ 和 $V _ { i + 1 }$ 之间的关系。
...
定义7（K维结构熵）。对于深度为 $K$ 的树 $T$ ，本文定义树的结构熵 $H$ 如下：
...
本节提出了一种异构图社区检测方法（SFCDH）。该方法打破了异构图的同质性结构，并增强了其同质性强度。SFCDH的核心思想是融合具有不同同质性比率的元路径子图，并利用动态掩码自编码器和对比学习进行异构图的非监督社区检测。通过融合元路径级节点嵌入和层次树级节点嵌入，SFCDH完成了非监督社区检测。

我们的模型架构如图3所示。SFCDH包含两个主要模块：基于同质性比率采样的层次结构融合模块（SFR）和基于生成式与对比学习的非监督图表示模块（UGC）。SFR由两部分组成：（a）从异构图中提取同质子图，并计算每个视图的同质性比率以生成高阶融合图。（b）构建高阶融合图的层次抽象树，并从树的角度学习节点特征。UGC由两部分组成：（c）应用掩码策略，基于高阶融合图和基于元路径的同质子图的社区原型计算对比损失。（d）使用异构掩码自编码器计算增强图在特征和结构重建前后的损失。通过整合元路径级和层次树级的节点嵌入，SFCDH实现了非监督社区检测。

# 4.1. 基于同质性比率采样的层级结构融合模块

SFR包含两个主要组成部分：基于同质性比率的元路径子图高阶重写以及层次化抽象树的构建。通过构建基于同质性比率的高阶融合图，前者提升了SFCDH泛化具有不同同质性比率的元路径子图的能力，增强了异质图中同质性边的影响，并削弱了异质性边的影响。后者通过构建层次化抽象树，从层次化社区中学习节点嵌入。
...
由于邻接矩阵 ?? 的不同阶次包含不同的结构信息，高阶图结构能够捕获更多关系。因此，使用公式(2)来计算 $A ^ { \phi }$ 的高阶邻域关系。

由于从不同元路径获得的同质子图具有不同的同质性比率，对高同质性比率的图结构进行高阶聚合将更有助于发现相似节点，而对低同质性比率的图结构进行高阶聚合则会导致不同类别节点的嵌入混淆。因此，我们设计了一种自适应聚合方法：对高同质性比率图采用高阶聚合，对低同质性比率图采用低阶聚合。由于无监督社区检测无法使用节点的真实标签，这里考虑使用伪标签 $R ^ { * }$ 来计算每个图的同质性比率，并通过计算同质性比率来确定每个图高阶聚合的最优阶次。
...
其中 $\lfloor \cdot \rfloor$ 是向下取整操作，$\delta$ 是用于过滤低同质性图的系数。为低同质性图分配较小的阶次，而为高同质性图分配高阶次。在伪标签的引导下，对每个视图进行精细化学习，以获得精细化的图结构。
...
编码树[36]将图以多粒度方式划分为层次化社区和子社区，有助于更好地理解图的结构。现有的异构图方法忽略了元路径间树的层次结构。在本节中，为了增强同质性强度并揭示各种层次结构，将生成的高阶融合图构建为树结构，并通过最小化结构熵来降低图结构的不稳定性。结构熵最小化的树结构定义为：
...
根据算法1，基于融合图 $S$ 构建层次化抽象树。首先，基于仅包含叶节点和根节点的融合图 $S$ 构建一棵树，使用 $S$ 的所有节点构成叶节点。第2-4行表示自底向上构建一棵二叉树。在这棵树中，
...
1: 构建包含根节点和叶节点的层次抽象树 $T$ ，且节点集合 $V$ 中的所有节点均为叶节点；   
2: 当 |????????.??ℎ????????????| $> 2$ 时执行   
...
其中 $\boldsymbol { x } _ { v } ^ { i }$ 表示编码树 $T$ 第??层节点 $v$ 的特征， $x _ { v } ^ { 0 }$ 是输入叶节点的特征，??.??ℎ???????????? 表示节点 $v$ 的子节点。最终从树的角度获得高阶融合图 $S$ 中所有节点的嵌入表示 $Z ^ { T r e e }$ 。
...
本文使用社区强度来衡量叶节点相对于树的角度的重要性得分。 $v ^ { - }$ 是节点 $v$ 的父节点。它由两部分组成： $T _ { v ^ { - } }$ 内部切割边的权重与图 $S$ 中所有节点度数的比值，以及树 $T _ { v ^ { - } }$ 中所有节点度数之和与整棵树度数之和的比值。该值可以解释为叶节点所属社区内部的边与该社区与其他社区之间连接边的比率。最终，层次抽象树角度节点表示为 $Z _ { v } ^ { T r e e } = o _ { v } Z _ { v } ^ { T r e e }$ ，其中 $Z _ { v } ^ { T r e e }$ 表示 $Z ^ { T r e e }$ 的第??行。

# 4.2. 面向联合生成学习和对比学习的无监督图表示学习模块

UGC模块由两部分组成：增强图的社区原型一致性模块和异构图掩码自编码器。前者采用基于社区原型的对比损失，结合约束动态掩码策略，对同质子图执行增强操作。同时，后者通过重构图结构和特征来学习深层节点嵌入。
...
现有的异构图对比学习方法很少考虑图社区结构。在本节中，我们考虑将动态掩码策略与社区结构相结合，并使用社区级对比损失来指导模型在训练过程中生成更具区分性的掩码，使得同一社区内的节点在不同增强图中具有相似的掩码策略。相反，不同社区中的节点具有不同的掩码策略，这有助于生成更具区分性的节点嵌入。对于给定的异构图 $G = \left( V , A , T _ { V } , T _ { E } , X , \phi \right) .$ ，为每条元路径 $\phi \in \Phi$ 构建基于元路径的邻接矩阵 $A ^ { \phi }$ 。每个 $A ^ { \phi }$ 通过伯努利分布 ???? $M _ { A } ^ { \phi } \sim$ Bernoulli $\left( p _ { e } \right)$ 生成一个用于图增强的二元掩码，其中 $p _ { e } < 1$ 表示掩码概率。然后，通过掩码 $\tilde { A } ^ { \phi ^ { 1 } } = M _ { A } ^ { \phi ^ { 1 } } \cdot A ^ { \phi } , \tilde { A } ^ { \bar { \phi } ^ { 2 } } = \bar { M _ { A } ^ { \phi ^ { 2 } } } \cdot A ^ { \phi } .$ 进行增强后，我们可以得到两个邻接矩阵。特征采用动态掩码方式，而非使用固定的掩码概率进行属性重建。属性掩码 $p _ { a }$ 的概率由调度函数 $\tau ( m + 1 ) = \tau ( m ) + { \cal { \Delta } }$ 获得，其中 $\tau \left( m \right)$ 表示在第 ?? 个训练周期中被掩码的特征比例。

对于 $\textit { m } \in \{ 0 , 1 , \ldots , M \}$ 、 $\tau ( 0 ) ~ = ~ m i n \left( p _ { a } \right)$ 和 $\tau \left( M \right) \ = \ m a x \left( p _ { a } \right)$ 。特征的二元掩码通过伯努利分布 $M _ { H } ^ { \phi } \sim$ Bernoulli $\left( p _ { a } \right)$ 生成，以得到扩展后的两个属性矩阵 $\tilde { X } ^ { \phi ^ { 1 } } = M _ { X } ^ { \phi ^ { 1 } } \cdot X ^ { \phi }$ 、 $\tilde { X } ^ { \phi ^ { 2 } } = M _ { X } ^ { \phi ^ { 2 } } \cdot X ^ { \phi }$ 。通过应用上述图的增强（分别记为 $t ^ { 1 }$ 和 $t ^ { 2 } \sim T$ 表示两次独立的增强），可以为每条元路径 $\phi \in \Phi$ 生成两个视图 $\left\{ \left( \tilde { X } ^ { \phi ^ { 1 } } , \tilde { A } ^ { \phi ^ { 1 } } \right) = t ^ { 1 } \left( X ^ { \phi } , \stackrel { \cdot } { A } { } ^ { \phi } \right) \right.$ 和 $\left( \tilde { X } ^ { \phi ^ { 2 } } , \tilde { A } ^ { \phi ^ { 2 } } \right) = t ^ { 2 } \left( X ^ { \phi } , A ^ { \phi } \right)$ 。

社区原型矩阵 Υ 在模型初始化时通过随机初始化获得。随后，在后续训练中，它由该社区所有节点的嵌入取平均得到：
...
其中 $\tilde { X } ^ { \phi ^ { 1 } }$ 表示图增强后与元路径 $\phi$ 对应的同构子图第一视图的属性矩阵，$\gamma \bar { \phi } ^ { 2 }$ 表示图增强后第二视图中的社区原型矩阵。

# 4.2.2. 异构图掩码自编码器

为捕捉异质图内部的复杂语义信息，我们对从元路径获得的同质子图$G _ { \phi } ~ = ~ \left( V ^ { \prime } , E ^ { \prime } \right)$和高阶融合图$S$进行掩码处理，此举破坏了短程语义连通性，有助于模型探索更深层关系以预测被掩码信息，并促进语义信息的更有效学习。为挖掘节点特征中包含的内容信息，对节点特征采用动态掩码策略。这使得模型能够在训练过程中逐步增强对目标类型节点特征的学习。

将属性矩阵$X$和掩码后的邻接矩阵$\widetilde { A } ^ { \phi }$输入编码器$f _ { E } \left( \cdot \right)$，即可获得学习到的节点特征。

$$
H _ {1} ^ {\phi} = f _ {E} \left(\widetilde {A} ^ {\phi}, X\right) \tag {17}
$$

随后使用解码器$f _ { D } \left( \cdot \right)$生成节点嵌入$H _ { 2 } ^ { \phi }$，通过sigmoid激活函数得到重构的邻接矩阵$A ^ { \phi ^ { \prime } }$，并计算重构邻接矩阵与原始邻接矩阵$A ^ { \phi }$之间的重构损失。
...
其中$l ^ { \phi }$表示元路径$\phi$的边重构损失，$\gamma _ { 1 }$为对应的比例因子。由于各元路径的重要性不同，此处引入语义级注意力机制来计算不同元路径的重要性得分$c ^ { \phi }$。通过使用softmax函数对$c ^ { \phi }$进行归一化，得到不同的元路径权重。
...
其中$\widetilde { V }$表示被掩码的目标节点集合，$\gamma _ { 2 }$为对应的比例因子。
...
17: 拼接 $Z$ 和 $Z ^ { T r e e }$   
18: 使用 K-means 算法获取社区分配矩阵 $R ^ { * }$   
19: 结束循环   
20: 返回 $R ^ { * }$

从异构图自编码器学习到的节点嵌入 $Z$ 与从层次抽象树视角学习到的节点嵌入 $Z ^ { T r e e }$ 进行拼接，实现了多级结构融合。基于轮次的社区分配矩阵 $R ^ { * }$ 通过 K-means 算法获得，在模型训练过程中作为生成高阶融合图的伪标签，直至模型达到最优状态。
...
为验证SFCDH的性能，本文使用6个公开可用的异构图数据集进行实验：ACM、DBLP、Freebase、IMDB、AMiner和Texas。表2描述了这6个异构图数据集。边数表示通过相应元路径提取的同质子图对应的边数量，第五列表示通过每条元路径提取的子图内部同配边的数量，第六列中的????表示同配比。

根据同配比，ACM、DBLP、Freebase和AMiner是典型的同配异构图，而IMDB和Texas是典型的异配异构图。

(1) ACM。一个常见的引文图数据集，包含三种节点类型：作者、论文和主题。   
(2) DBLP。一个常见的引文图数据集，包含四种节点类型：作者、论文、术语和会议。   
(3) Freebase。一个常见的大型电影图数据集，包含四种节点类型：电影、演员、导演和编剧。   
(4) IMDB。一个在线电影图数据集，包含四种节点类型：电影、演员、导演和关键词。   
(5) AMiner。一个在线引文图数据集，包含三种节点类型：论文、作者和参考文献。目标节点由基于原始数据集子集划分为四类的论文组成。   
(6) Texas。为测试SFCDH在异配图上的性能，使用了Texas和IMDB等数据集。Texas是来自WebKB3数据集的网页图，其同配比(HR)为0.06。

# 5.1.2. 基线

本研究将所提出的SFCDH模型与11个基线模型进行了比较，这些基线模型分为三类：同质网络社区检测方法（DFCN、VGAER、DDGAE）、异质网络社区检测方法（HAN、HeCo、HetGNN-SF、HGMAE、HAESF）以及异嗜网络社区检测方法（HoLe、AHGFC、PLCSR）。以下简要介绍这些对比模型。

HAN [21] 融合了节点级注意力机制，用于识别节点及各类邻居的重要性，同时采用语义级注意力机制评估不同元路径的相关性，最终利用节点嵌入进行社区检测。

DFCN [37] 采用动态融合模块，将局部与全局视角下获取的嵌入进行合并，并集成三重自监督机制以增强社区检测性能。

HeCo [38] 在网络模式视图与元路径视图之间执行直接的跨视图对比学习，通过视图间的相互监督学习鲁棒的节点嵌入。

VGAER [39] 采用变分图自编码器进行社区检测，将高阶模块度信息与节点特征相结合。

HetGNN-SF [40] 利用加权融合方法构建语义融合图，从语义强度与特征相似性中推导出最终节点嵌入，并结合对比学习进行优化。

HGMAE [6] 借助动态掩码自编码器，通过编码属性、拓扑及位置信息学习节点嵌入，从而有效促进社区检测。

HoLe [41] 采用层次相关估计与聚类感知稀疏模块增强图同质性，将社区检测与图结构学习有效整合。

DDGAE [42] 利用高阶模块度与属性信息构建视图，通过深度多重注意力机制，基于拓扑与特征重构进行社区检测。

HAESF [5] 采用图自编码器聚合异质图结构信息，并利用非负矩阵分解学习社区语义信息，同时对两者进行联合优化。

AHGFC [33] 基于图同质性，通过考虑低频与高频信号增强节点嵌入，并利用节点特征与邻接关系设计图联合聚合矩阵，使这些信号更具区分性。

PLCSR [43] 采用基于指数移动平均的自辅助编码器与动态阈值驱动的课程选择策略，提升伪标签的可靠性，从而在图学习模型中实现精确的节点处理以进行社区检测。
...
本节旨在基于PyTorch实现所提出的SFCDH模型。模型参数通过随机初始化生成，编码器和解码器均采用HAN，学习率范围为8e-6至3e-4，并使用Adam优化器进行训练。早停耐心值范围为10至50。模型超参数根据在不同数据集上的实验进行调整。所有实验均在RTX 4090 GPU上进行。

对于基线模型，参数默认遵循原始论文的设置。在最终学习到的节点嵌入中，使用K-means算法进行社区检测，并取10次不同随机种子运行结果的平均值。

# 5.2. 性能分析

在进行社区检测之前，我们基于同构图的基线方法，通过元路径将异构图表数据集转换为同构图。表3至表8展示了社区检测对6个数据集的影响，包括基线方法和所提出的模型。表3比较了DBLP数据集上的结果，显示SFCDH的NMI得分分别超过HAN、DFCN、HeCo、VGAER、HetGNN-SF、HGMAE、HoLe、DDGAE、HAESF、AHGFC和PLCSR $1 4 . 9 8 \%$、$1 1 . 4 2 \%$、$5 \%$、$4 . 6 4 \%$、$1 4 . 6 7 \%$、$4 . 1 \%$、$3 5 . 1 2 \%$、$1 2 . 8 \%$、$3 . 6 9 \%$、$1 8 . 5 1 \%$和$3 9 \%$。这表明所提出的方法显著优于最近的异构图表社区检测技术（如HGMAE、HetGNN-SF和HAESF）、同构图检测方法（如DDGAE和DFCN）以及异质图检测方法（包括HoLe、AHGFC和PLCSR）。
...
表5比较了Freebase数据集上的结果，显示SFCDH在NMI、ACC和精确度指标上取得了最高得分。尽管在其他指标上排名仅次于HetGNN-SF和VGAER，但SFCDH的性能仍然具有很强的竞争力。

表6至表8展示了IMDB和Texas数据集上的实验结果，这些数据集以异构图表中的异质性为特征。由于IMDB和Texas缺乏正负样本信息，HeCo无法作为基线使用。其余10个基线方法的结果较差。IMDB数据集的异质性导致无法有效分析从元路径中提取的同构子图，从而产生次优结果。然而，SFCDH在准确度上比排名第二的模型高出$1 1 \%$。这些结果表明，所提出的模型在异质性和同质性异构图表上均表现良好。
...
由于本文模型由多个模块组成，本小节通过逐一移除各模块来分析不同模块对模型的贡献。SFCDH-C、SFCDH-G和SFCDH-T分别表示移除基于社区原型的图对比机制、移除高阶融合图以及移除层次抽象树。图5展示了在ACM和DBLP数据集上的实验结果。特别地，移除高阶融合图对社区检测的影响最大。移除社区原型对比机制和层次抽象树也会导致模型性能下降，表明它们在提升模型性能方面的有效性。整合所有模块可获得最佳效果。

# 5.4. 参数分析

在本节中，针对ACM、DBLP和IMDB数据集，对5个主要参数进行了敏感性实验。

**损失函数平衡系数分析。** 为评估模型的鲁棒性，首先调整了损失函数的平衡系数。平衡系数决定了训练过程中三个损失函数的权重。系数??的取值范围为1至10，调整间隔为1。图6展示了在ACM数据集上的实验结果。可以发现，当?? $= 2$ 时，模型达到最佳结果。当NMI在1–10范围内时，模型性能呈现先上升、后下降、再上升的趋势，整体未出现显著下降。在此范围内，NMI在0.58至0.63之间波动，优于基线。该图表明，SFCDH的性能保持相对稳定，体现了鲁棒性，并验证了损失函数 $L _ { \mathrm { C o s } }$ 的有效性。总体而言，随着系数 $\mu$ 的增加，模型有效性略有下降，整体波动幅度约为 $2 \%$ 。随着系数的增加，参数 $\eta$ 对模型性能呈现整体上升趋势，表明SFCDH在较高的属性重构损失下表现更优。

**隐藏维度分析。** 为评估不同隐藏维度对模型性能的影响，如图7所示，不同的隐藏维度对DBLP和IMDB数据集的模型性能产生影响。我们将隐藏层维度定义为64、128、256、512、1024。可以发现，使用过小的隐藏层维度会导致模型无法捕获全面信息，而过大的隐藏层维度则会分散模型对有效信息的注意力，因此过小或过大的隐藏层维度都会降低模型性能。然而，适中的隐藏层维度足以使模型达到最优性能。

**层次抽象树高度分析。** 在DBLP和IMDB数据集上，我们不断调整层次抽象树的高度，以考察其对模型性能的影响。根据图8，DBLP和IMDB数据集分别在树高为3和5时达到最优性能。当树高过大或过小时，模型性能均会下降。过大的树高可能导致数据在每层传递过程中信息丢失或模糊，造成不同社区节点嵌入之间的混淆。反之，若树高过低，则无法捕获图的层次社区信息，也无法学习细粒度的节点嵌入。

**网络层分析的影响。** 为探究网络层深度对模型效能的影响，我们在IMDB数据集上系统性地将网络层数在2至6的范围内进行变化。分析表明，随着网络层数的逐步调整，包括ACC、NMI、ARI和F1分数在内的性能指标波动极小。值得注意的是，模型呈现出趋于稳定的趋势，表明其对网络层数的变化具有鲁棒性。研究结果表明，模型在不同网络层配置下表现出一致的性能，对层深变化的敏感性较低。这种稳定性凸显了模型对该参数变化的适应能力，强调了其在多样化设置中的可靠性（见图9）。

**动态掩码策略的影响。** 我们在为增强图中社区原型一致性设计的模块中实现了动态掩码率。为评估其有效性，我们进行了综合分析，比较了动态掩码率 $p$ 与固定掩码率的性能，如图10所示。我们的评估聚焦于在固定掩码率（0.4至0.8）和动态掩码 $p$ 下NMI、ARI和ACC的变化。结果表明，动态掩码始终优于固定掩码率。此外，我们观察到随着掩码率的增加，性能总体提升，表明最优掩码率有助于增强模型有效性。同时，动态掩码机制使模型能够高效学习，无需手动调整掩码率。

# 5.5. 阐释社区可解释性

为了展示所获取社区的可解释性，我们通过热图可视化了这些社区的全局表示矩阵。图11展示了模型在四个不同社区中学到的DBLP数据集的特征分布，以及每个社区节点的表达偏好。热图中的颜色梯度表示每个社区内语义特征的强度，越接近黄色表示语义相关性越高。最亮的颜色代表社区的主要特征，而稍暗的色调则代表次要特征。值得注意的是，不同社区的主要特征与次要特征之间存在显著对比，凸显了它们之间的清晰区别。从图11中，可以辨别出每个社区的全局特征及其代表性特点。例如，以关键特征集中为特征的社区往往成为意见领袖，有助于有效的社区管理和信息治理。相反，主要特征分散的社区可能难以统一意见，导致社区凝聚力松散，从而对社区稳定性产生负面影响。总之，SFCDH模型推导出的社区具有高度的可解释性，这是实际应用中的一个关键因素。
...
图12展示了AMiner和Texas数据集的损失趋势。最初，Texas数据集的损失相当高，但在早期阶段迅速降至最小值。相比之下，Aminer数据集的损失表现出轻微但持续的波动。最终，两个数据集的损失都趋于稳定并收敛。
...
本节分析了ACM和IMDB数据集在基于伪标签的模型训练过程中的同质性比率。在图13(a)中，$\mathrm { H R } _ { 0 }$、$\mathrm { H R } _ { 1 }$和$\mathrm { H R } _ { S }$分别代表ACM数据集中元路径PAP和PSP对应的同质子图，以及高阶融合图$S$的同质性比率。随着训练轮次的增加，我们可以观察到$\mathrm { H R } _ { 0 }$上升而$\mathrm { H R } _ { 1 }$下降。然而，这些比率均优于基于数据集原始标签推导出的同质性比率。

![](images/dd6921243a507131816e22d8bb386735a5a9a2135b02a155fc065df97dd37292.jpg)  
图11. 社区可解释性。

模型中的同质性比率存在一定程度的波动，但变化范围不大，整体效果保持稳定。由于同质子图的同质性比率随训练轮次增加而下降，$\mathrm { H R } _ { S }$低于$\mathrm { H R } _ { 0 }$的上升幅度和$\mathrm { H R } _ { 1 }$。图13(b)显示，随着训练轮次增加，由$\mathrm { H R } _ { 0 }$、$\mathrm { H R } _ { 1 }$和$\mathrm { H R } _ { 2 }$表示的IMDB数据集中元路径MAM、MDM和MKM对应的同质子图同质性比率均呈下降趋势。这一趋势反映了IMDB数据集的异质性特征。如表2所示，该数据集在多个元路径子图中表现出较低的同质性比率。由于模型在初始阶段效果不佳，无法区分属于不同社区的节点，因此图中许多异质边被识别为同质边。因此，在预训练阶段模型的同质性比率异常偏高。然而，随着训练轮次增加，SFCDH模型的有效性提升，社区边界变得更加清晰。这种区分导致同质性比率逐渐下降，并逐步接近但始终高于实际同质性比率。上述分析证明了使用伪标签聚合元路径子图的合理性和有效性。

# 5.8. 可视化



为了更直观地理解模型结果，本文采用t-SNE技术对不同模型的结果进行可视化，并在DBLP数据集上比较了VGAER、HeCo、HetGNN-SF以及本文提出的SFCDH模型的结果。如图14所示，不同颜色代表不同社区。在图14(a)中，VGAER中的几个社区存在重叠，且同一社区的节点过于离散，无法聚合。在图14(b)中，HeCo能够区分不同社区的节点，但同一社区的节点无法聚合。在图14(c)中，HetGNN-SF能够区分大部分不同社区的节点，但许多同一社区的节点无法聚合在一起，导致相对孤立的状态，同时社区之间存在边界不清晰的重叠现象。图14(d)展示了本文提出的SFCDH模型，其中属于同一社区的节点凝聚成更密集的簇，社区之间的边界变得更加清晰。
...
为了分析模型的复杂度，我们首先定义几个常数：$g$ 表示由元路径组成的同质子网络数量，$n$ 表示目标节点数量，?? 表示边数，$d$ 表示每个节点的特征维度。算法2的时间复杂度主要由以下部分组成：构建高阶融合图、构建层次抽象树、扩展同质子图和高阶融合图、使用异构掩码自编码器计算属性和特征重构损失，以及最终使用K-means算法获取社区分配矩阵。高阶融合图的构建涉及矩阵乘法，其复杂度为 $O \left( g \times n ^ { 2 } \right)$；层次抽象树的构建时间复杂度为 $O ( n \times \log ^ { 2 } \mathfrak { n } ) )$；掩码节点属性和计算社区原型损失 $L _ { \mathrm { { C o s } } }$ 的时间复杂度为 $O ( g \times ( e + n \times k \times$ ??)。异构掩码自编码器的时间复杂度为 $O \left( g \times \left( e \times d + n \times d ^ { 2 } \right) \right)$。因此，算法2的总复杂度为 $O \left( g \times \left( e + n \times k \times d + e \times d + n \times d ^ { 2 } \right) + n \times \log ^ { 2 } \mathrm { n } + g \times n ^ { 2 } \right)$。

为了比较这12种方法的运行效率，我们在表9中展示了AMiner和Texas数据集上社区检测算法的总运行时间（以秒为单位）。与异嗜性网络社区检测方法相比，SFCDH的运行时间更短，但其运行效率与同构网络或异构网络社区检测方法相当或略低。这是因为现有的异嗜性网络社区检测方法往往在模型训练过程中不断更新原始图结构，这一过程较为耗时。然而，结合表3至表8的实验结果，考虑到模型的整体有效性，SFCDH相比其他方法仍具有显著优势。
...
本文首次尝试解决异质性异构图中的节点嵌入坍塌问题。提出了一种名为SFCDH的无监督社区检测方法，该方法具有特征多级结构融合能力，并对同质图和异质图均具备强泛化性能。具体而言，SFCDH提出基于同质性比率采样（SFR）的层级结构融合模块，用于构建细粒度的高阶融合图，进而从树结构视角提取节点嵌入，使其能够泛化至具有不同同质性比率的元路径子图。此外，为增强异构图中的目标节点特征学习与复杂语义理解，本文提出了一种结合社区原型对比学习与动态掩码自编码器的无监督图表示学习模块（UGC）。最终，将包括元路径视角与树结构视角在内的多层级节点嵌入进行融合，用于社区检测。通过联合优化，本文在包含异质与同质异构图的6个基准数据集上证明了SFCDH相较于现有最优基线的优越性。

然而，未来可开展进一步研究以提升SFCDH的性能。首先，本文的属性重构损失依赖于目标类型节点的特征，但在异构图中可能存在这些特征缺失的情况。尽管图结构信息有助于弥补这一不足，未来工作可聚焦于开发能有效处理节点特征缺失的社区检测方法。其次，本文仅研究了节点的异质性，未考虑边的异质性。边具有不同的相关性与重要性等级，在分析图时必须纳入考量。此外，应考虑将该方法扩展至动态异构图，以检测社区结构，并研究社区随特征、节点与结构动态演变时的演化规律。

# CRediT 作者贡献声明

孟莹戴：撰写 – 初稿，可视化，方法论，数据整理，概念构思。李伟民：撰写 – 审阅与编辑，监督，资源，概念构思。张欣怡：撰写 – 审阅与编辑。刘芳芳：监督，形式分析。辛明军：撰写 – 审阅与编辑，项目管理。王灿：撰写 – 审阅与编辑，验证。
...
本研究受国家重点研发计划（编号：2022YFC3302600）资助。本研究还得到上海大学上海科学与工程计算技术服务中心的支持。
...
[4] L. Luo, Y. Fang, X. Cao, X. Zhang, W. Zhang, 从异构图检测社区：一种基于上下文路径的图神经网络模型, 载于：第30届ACM国际信息与知识管理会议论文集, 2021, 第1170–1180页.   
[5] Y. Zhao, W. Li, F. Liu, J. Wang, A.M. Luvembe, 集成异构结构与社区语义用于异构网络中的无监督社区检测, Expert Syst. Appl. 238 (2024) 121821.   
[6] Y. Tian, K. Dong, C. Zhang, C. Zhang, N.V. Chawla, 异构图掩码自编码器, 载于：AAAI人工智能会议论文集, 37, (8) 2023, 第9997–10005页.   
[7] S. Luan, C. Hua, Q. Lu, J. Zhu, M. Zhao, S. Zhang, X.-W. Chang, D. Precup, 重新审视图神经网络的异质性, Adv. Neural Inf. Process. Syst. 35 (2022) 1362–1375.   
[8] X. Shi, H. Lu, Y. He, S. He, 基于成对约束对称非负矩阵分解的社交网络社区检测, 载于：2015年IEEE/ACM社会网络分析与挖掘国际会议论文集, 2015, 第541–546页.   
[9] D. He, X. You, Z. Feng, D. Jin, X. Yang, W. Zhang, 一种面向社区检测的网络特定马尔可夫随机场方法, 载于：AAAI人工智能会议论文集, 32, (1) 2018.   
[10] G. Bonifazi, F. Cauteruccio, E. Corradini, M. Marchetti, A. Pierini, G. Terracina, D. Ursino, L. Virgili, 一种检测社交平台不同社区间信息传播者骨干的方法, Data Knowl. Eng. 140 (2022) 102048.   
...
[13] Q. Kong, J. Sun, Z. Xu, 面向属性网络社区检测的联合正交对称非负矩阵分解, Knowl.-Based Syst. 283 (2024) 111192.   
[14] C. Li, X. Guo, W. Lin, Z. Tang, J. Cao, Y. Zhang, 基于模体感知的多层网络社区检测算法, Knowl.-Based Syst. 260 (2023) 110136.   
[15] Y. Liu, Z. Liu, X. Feng, Z. Li, 保留社区信息的鲁棒属性网络嵌入, 载于：2022年IEEE第38届国际数据工程会议, ICDE, IEEE, 2022, 第1874–1886页.   
...
[18] B. Li, B. Jing, H. Tong, 图社区对比学习, 载于：2022年ACM网络会议论文集, 2022, 第1203–1213页.   
[19] Y. Sun, Y. Yu, J. Han, 基于星型网络模式的异构信息网络排序聚类, 载于：第15届ACM SIGKDD国际知识发现与数据挖掘会议论文集, 2009, 第797–806页.   
[20] Z. Li, Z. Pan, Y. Zhang, G. Li, G. Hu, 等, 异构社交网络中的高效社区检测, Math. Probl. Eng. 2016 (2016).   
[21] X. Wang, H. Ji, C. Shi, B. Wang, Y. Ye, P. Cui, P.S. Yu, 异构图注意力网络, 载于：万维网会议, 2019, 第2022–2032页.   
[22] C. Zhang, D. Song, C. Huang, A. Swami, N.V. Chawla, 异构图神经网络, 载于：第25届ACM SIGKDD国际知识发现与数据挖掘会议论文集, 2019, 第793–803页.

[23] S. Yun, M. Jeong, R. Kim, J. Kang, H.J. Kim, 图变换网络, Adv. Neural Inf. Process. Syst. 32 (2019).   
[24] D. He, D. Liu, D. Jin, W. Zhang, 一种检测复杂网络中异构链接社区的随机模型, 载于：AAAI人工智能会议论文集, 29, (1) 2015.   
[25] H. Zhao, P. Rui, J. Chen, Y. Zhang, Y. Wang, S. Zhao, J. Tang, HINCHIP：保留社区层次结构的异构信息网络表示, Knowl.-Based Syst. 264 (2023) 110343.   
[26] H. Wei, G. Xiong, Q. Wei, W. Cao, X. Li, 结构感知的属性异构图网络嵌入, Knowl. Inf. Syst. 65 (4) (2023) 1769–1785.   
[27] L. Yang, M. Li, L. Liu, C. Wang, X. Cao, Y. Guo, 等, 面向异质性属性的多样化消息传递, Adv. Neural Inf. Process. Syst. 34 (2021) 4751–4763.   
[28] X. Li, R. Zhu, Y. Cheng, C. Shan, S. Luo, D. Li, W. Qian, 在图神经网络中遇到异质性时寻找全局同质性, 载于：国际机器学习会议, PMLR, 2022, 第13242–13256页.   
[29] E. Chien, J. Peng, P. Li, O. Milenkovic, 自适应通用广义PageRank图神经网络, 载于：国际学习表征会议, 2021.   
[30] Y. Wu, Y. Wang, L. Hu, J. Hu, DCGNN：面向异质性图的自适应深度图卷积, Inform. Sci. 666 (2024) 120427.   
[31] B.-M. Liu, Y.-L. Gao, F. Li, C.-H. Zheng, J.-X. Liu, SLGCN：增强结构的线图卷积网络用于预测药物-疾病关联, Knowl.-Based Syst. 283 (2024) 111187.   
[32] Y. Zheng, H. Zhang, V.C. Lee, Y. Zheng, X. Wang, S. Pan, 寻找缺失的一半：面向同质性倾向和异质性倾向图的图互补学习, 载于：第40届国际机器学习会议论文集, 载于：ICML’23, JMLR.org, 2023, 第14页, 1788.   
[33] Z. Wen, Y. Ling, Y. Ren, T. Wu, J. Chen, X. Pu, Z. Hao, L. He, 同质性相关：面向多视图图聚类的自适应混合图滤波器, 载于：AAAI, 2024, 第15841–15849页.

[34] D. Zou, H. Peng, X. Huang, R. Yang, J. Li, J. Wu, C. Liu, P.S. Yu, Se-gsl：一种通过结构熵优化的通用有效图结构学习框架, 载于：2023年ACM网络会议论文集, 2023, 第499–510页.   
[35] Y. Dong, N.V. Chawla, A. Swami, Metapath2vec：面向异构网络的可扩展表示学习, 载于：第23届ACM SIGKDD国际知识发现与数据挖掘会议论文集, 2017, 第135–144页.   
[36] A. Li, X. Yin, B. Xu, D. Wang, J. Han, Y. Wei, Y. Deng, Y. Xiong, Z. Zhang, 通过图结构熵从超低分辨率Hi-C数据解码拓扑关联结构域, Nat. Commun. 9 (1) (2018) 3265.   
[37] W. Tu, S. Zhou, X. Liu, X. Guo, Z. Cai, E. Zhu, J. Cheng, 深度融合聚类网络, 载于：AAAI人工智能会议论文集, 35, (11) 2021, 第9978–9987页.   
[38] X. Wang, N. Liu, H. Han, C. Shi, 基于共对比学习的自监督异构图神经网络, 载于：第27届ACM SIGKDD知识发现与数据挖掘会议论文集, 2021, 第1726–1736页.   
[39] C. Qiu, Z. Huang, W. Xu, H. Li, VGAER：基于图神经网络重构的社区检测, 载于：AAAI: DLG-AAAI’22, 2022.   
[40] C. Li, X. Liu, Y. Yan, Z. Zhao, Q. Zeng, Hetgnn-SF：基于语义强度和特征相似性的异构图神经网络自监督学习, Appl. Intell. 53 (19) (2023) 21902–21919.   
[41] M. Gu, G. Yang, S. Zhou, N. Ma, J. Chen, Q. Tan, M. Liu, J. Bu, 增强同质性的结构学习用于图聚类, 载于：第32届ACM国际信息与知识管理会议论文集, 2023, 第577–586页.   
[42] X. Wu, W. Lu, Y. Quan, Q. Miao, P.G. Sun, 深度双图注意力自编码器用于社区检测, Expert Syst. Appl. 238 (2024) 122182.   
[43] P. Zhu, J. Li, Y. Wang, B. Xiao, J. Zhang, W. Lin, Q. Hu, 通过课程自我反思提升伪标签用于属性图聚类, IEEE Trans. Neural Networks Learn. Syst. (2024) 1–14.